...
class Homework4:
    ...
    def randomQuickSort(self, nums: list) -> list:
        ...
        """
        Public entry point for Randomized Quicksort.
        Copies the input so the caller's list is never mutated,
        then delegates to the recursive driver.
        """
        result = nums[:]
        self._quick_sort_helper(result, 0, len(result) - 1)
        return result

    def _quick_sort_helper(self, seq, lo, hi):
        """
        Recursive driver that keeps partitioning the array
        until every sub-segment has one or zero elements.
        """
        if lo >= hi:
            return
    ...
    def _partition_random(self, seq, lo, hi):
        """
        ...
        seq[lo], seq[chosen] = seq[chosen], seq[lo]

        pivot_val = seq[lo]
        wall = hi + 1

        # Walk backwards, pushing elements >= pivot toward the right end
        for cursor in range(hi, lo, -1):
            if seq[cursor] >= pivot_val:
                wall -= 1
                seq[cursor], seq[wall] = seq[wall], seq[cursor]

        # Drop the pivot into its correct slot
        wall -= 1
        seq[lo], seq[wall] = seq[wall], seq[lo]
        return wall
    ...
    def heapSort(self, nums: list) -> list:
        ...
        """
        Heapsort driver.
        Builds a max-heap in-place, then repeatedly pulls the
        largest element to the back of the array.
        """
        data = nums[:]
        length = len(data)
